...
Alpaca MCP Proxy
================
Bearer-guarded HTTP proxy in front of the upstream `alpaca-mcp-server`
running locally on 127.0.0.1:8010. The upstream is FastMCP-based and
exposes 43 tools across orders, positions, watchlists, market data,
portfolio history, and corporate actions.

Architecture:
  CoWork  --Bearer MCP_API_KEY-->  alpaca.bibager.com/mcp     (this proxy on 8009)
  proxy   --(no auth header)   --> 127.0.0.1:8010/mcp         (alpaca-mcp-server,
                                                              localhost-only)
  alpaca-mcp-server --(ALPACA_API_KEY, ALPACA_SECRET_KEY)--> Alpaca API

Why a proxy in front:
  - alpaca-mcp-server has NO built-in auth. If we exposed it directly,
    anyone who learned the URL could place trades against this account.
  - We bind alpaca-mcp-server to 127.0.0.1 so it's never reachable from
    outside the container, then wrap it with our Bearer-MCP_API_KEY guard
    + synthetic OAuth — the same security model as every other service.

The MCP OAuth flow at the gateway is synthetic: it auto-approves and
issues MCP_API_KEY as the access_token so Claude CoWork's stored token
never expires.

Env vars:
  MCP_API_KEY      required; protects the gateway side
  ALPACA_UPSTREAM  optional; defaults to http://127.0.0.1:8010/mcp
  PORT             optional, defaults to 8009
...
ALPACA_UPSTREAM: str = os.getenv("ALPACA_UPSTREAM", "http://127.0.0.1:8010/mcp")
...
# Headers we strip from the inbound request. We strip Authorization too
# because alpaca-mcp-server doesn't expect any auth header (it reads keys
# from env at startup) — and our MCP_API_KEY would just confuse it.
...
async def proxy_mcp(request: Request) -> StreamingResponse:
    """Stream-forward any /mcp request to the local alpaca-mcp-server."""
    upstream_headers = {
        k: v for k, v in request.headers.items()
        if k.lower() not in _REQ_STRIP
    }
    # Note: NO Authorization header sent upstream — alpaca-mcp-server is
    # localhost-only and reads ALPACA_API_KEY/ALPACA_SECRET_KEY from env.
...
if __name__ == "__main__":
    logger.info("Alpaca MCP proxy starting on port %d (upstream: %s)", PORT, ALPACA_UPSTREAM)
