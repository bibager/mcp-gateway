"""
Unusual Whales MCP Proxy
========================
Transparent HTTP-streaming proxy for the upstream Unusual Whales MCP server
(https://api.unusualwhales.com/api/mcp). The MCP session lives directly between
Claude CoWork and Unusual Whales — we just rewrite the Authorization header.

NOTE: Unusual Whales' public docs URL `unusualwhales.com/public-api/mcp` is
the documentation page, NOT the MCP endpoint. The real endpoint is
`api.unusualwhales.com/api/mcp` per their setup guide.

Flow:
  CoWork  --Bearer MCP_API_KEY-->  uw.bibager.com/mcp
  gateway --Bearer UW_API_KEY  --> api.unusualwhales.com/api/mcp
...
Env vars:
  ...
  UW_API_KEY     required; upstream Unusual Whales API token (UUID)
  UW_UPSTREAM    optional; defaults to https://api.unusualwhales.com/api/mcp
  PORT           optional, defaults to 8012
...
logger = logging.getLogger("uw_mcp")

# --- Config ------------------------------------------------------------------

MCP_API_KEY: str = os.environ["MCP_API_KEY"]
UW_API_KEY: str = os.environ["UW_API_KEY"]
UW_UPSTREAM: str = os.getenv("UW_UPSTREAM", "https://api.unusualwhales.com/api/mcp")
...
async def proxy_mcp(request: Request) -> StreamingResponse:
    """Stream-forward any /mcp request to Unusual Whales with our Bearer token."""
    upstream_headers = {
        k: v for k, v in request.headers.items()
        if k.lower() not in _REQ_STRIP
    }
    upstream_headers["Authorization"] = f"Bearer {UW_API_KEY}"
...
if __name__ == "__main__":
    logger.info("Unusual Whales MCP HTTP proxy starting on port %d (upstream: %s)", PORT, UW_UPSTREAM)
